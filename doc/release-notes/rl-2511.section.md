...
- `lima` package now only includes the guest agent for the host's architecture by default. If the guest VM's architecture differs from the Lima host's, use `lima-full` package.
